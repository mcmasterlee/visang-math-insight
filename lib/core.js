...
},required:['id','label','evidence','needsReview','reviewEvidence','hasImprovementPoint','improvementCategory','improvementEvidence','userClass']}}},required:['summary','topics','sentiments']};
  const instructions=`당신은 한국 교육교재 VOC 분석가입니다. 제공된 게시글 각각을 실제 문장 근거로 긍정/중립/부정 중 하나로 분류하세요. 광고·공식 소개 문구는 감성 근거로 사용하지 마세요. evidence는 해당 감성 판단의 근거가 된 제공 문장 속 짧은 표현을 25자 이내로 작성하세요. 별도로 needsReview는 전체 감성이 긍정/중립이더라도 교재의 단점, 불편, 부족함, 난이도 부담, 오류, 아쉬움처럼 담당자가 검토할 가치가 있는 부정적 요소가 실제 문장에 포함되면 true로 지정하세요. 단순 정보 공유나 추측만으로는 true로 지정하지 마세요. reviewEvidence에는 needsReview=true의 직접 근거 표현을 25자 이내로 작성하고 false면 빈 문자열로 작성하세요. 또한 hasImprovementPoint는 글 전체가 좋다는 평가여도 '아이에게 어렵다', '설명이 부족하다', '문제량이 아쉽다', '해설이 불편하다'처럼 비상교육 교재 개선에 참고할 수 있는 구체적 문제점이 단 한 문장이라도 있으면 true로 지정하세요. improvementCategory는 난이도/개념 설명/문제 구성/문제량/해설/편집·가독성/학습 부담/정답·부가자료/기타 중 가장 직접적인 하나를 선택하고, 없으면 '없음'으로 지정하세요. improvementEvidence에는 실제 제공 문장에서 개선 판단의 직접 근거가 되는 표현을 40자 이내로 작성하고, 없으면 빈 문자열로 작성하세요. userClass는 화면에 공개할 사용자 관점 분류입니다. 실제 사용 경험을 바탕으로 추천, 만족, 재구매 의사, 학습 효과, 아이가 잘 푼다는 등 뚜렷한 호평이 중심이면 high_preference로 분류하세요. 구체적인 사용 평가 없이 자료 공유, 출간 정보, 구매 문의, 파일 요청, 단순 기록 중심이면 neutral로 분류하세요. 전체적으로 좋은 글이어도 '아이가 어려워한다', '설명이 부족하다', '문제량이 아쉽다', '해설이 불편하다', '이 부분은 개선되면 좋겠다'처럼 교재 개선에 참고할 구체적 아쉬움이 하나라도 핵심적으로 확인되면 improvement_request로 분류하세요. improvement_request는 '부정적 사용자'라는 의미가 아니라 제품 개선에 도움이 되는 솔직한 사용 의견이라는 관점으로 판정하세요. summary는 전체 후기의 반복 장점, 단점, 활용 맥락을 근거 중심으로 4~6문장으로 요약하세요. topics는 실제 반복 주제만 작성하세요.`;
  ...
  for(const p of merged){
    ...
    if(sx){
      ...
      p.userClass=sx.userClass||(
        sx.hasImprovementPoint?'improvement_request':
        sx.label==='positive'?'high_preference':'neutral'
      );
    }
  }
