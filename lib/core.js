...
async function analyzeWithOpenAI(level,book,posts,cfg){
  ...
  const schema={type:'object',additionalProperties:false,properties:{summary:{type:'string'},topics:{type:'array',minItems:3,maxItems:7,items:{type:'string'}},sentiments:{type:'array',items:{type:'object',additionalProperties:false,properties:{id:{type:'string'},label:{type:'string',enum:['positive','neutral','negative']},evidence:{type:'string'},needsReview:{type:'boolean'},reviewEvidence:{type:'string'}},required:['id','label','evidence','needsReview','reviewEvidence']}}},required:['summary','topics','sentiments']};
  const instructions=`당신은 한국 교육교재 VOC 분석가입니다. 제공된 게시글 각각을 실제 문장 근거로 긍정/중립/부정 중 하나로 분류하세요. 광고·공식 소개 문구는 감성 근거로 사용하지 마세요. evidence는 해당 감성 판단의 근거가 된 제공 문장 속 짧은 표현을 25자 이내로 작성하세요. 별도로 needsReview는 전체 감성이 긍정/중립이더라도 교재의 단점, 불편, 부족함, 난이도 부담, 오류, 아쉬움처럼 담당자가 검토할 가치가 있는 부정적 요소가 실제 문장에 포함되면 true로 지정하세요. 단순 정보 공유나 추측만으로는 true로 지정하지 마세요. reviewEvidence에는 needsReview=true의 직접 근거 표현을 25자 이내로 작성하고 false면 빈 문자열로 작성하세요. summary는 전체 후기의 반복 장점, 단점, 활용 맥락을 근거 중심으로 4~6문장으로 요약하세요. topics는 실제 반복 주제만 작성하세요.`;
...
function publicRecord(level,book,record){
  // 기존 Supabase에 남아 있는 예전 오탐 데이터도 조회 시 즉시 다시 검증해 숨긴다.
  ...
  const analysis=oldAnalysis ? {...oldAnalysis,keywords:freshKeywords} : (posts.length?{
    summary:deriveFallbackSummary(posts,freshKeywords),positive:null,neutral:null,negative:null,
    topics:freshKeywords.slice(0,5).map(k=>k.word),keywords:freshKeywords,aiStatus:'off',aiError:null
  }:null);
  return {level,book,mentions:posts.length,today:posts.filter(p=>(p.firstSeenAt||'').slice(0,10)===todayKst()).length,posts,analysis,lastUpdated:record?.lastUpdated||null,filterStats:record?.filterStats||null};
}
