...
async function analyzeWithOpenAI(level,book,posts,cfg){
  ...
  const schema={type:'object',additionalProperties:false,properties:{summary:{type:'string'},topics:{type:'array',minItems:3,maxItems:7,items:{type:'string'}},sentiments:{type:'array',items:{type:'object',additionalProperties:false,properties:{id:{type:'string'},label:{type:'string',enum:['positive','neutral','negative']},evidence:{type:'string'}},required:['id','label','evidence']}}},required:['summary','topics','sentiments']};
  const instructions=`당신은 한국 교육교재 VOC 분석가입니다. 제공된 게시글 각각을 실제 문장 근거로 긍정/중립/부정 중 하나로 분류하세요. 광고·공식 소개 문구는 감성 근거로 사용하지 마세요. evidence는 해당 판단의 근거가 된 제공 문장 속 짧은 표현을 25자 이내로 작성하세요. summary는 전체 후기의 반복 장점, 단점, 활용 맥락을 근거 중심으로 4~6문장으로 요약하세요. topics는 실제 반복 주제만 작성하세요.`;
